...
class TestCoder:
    ...
    async def _drain_show_send_output_stream(self, coder, completion):
        self._reset_partial_response_state(coder)
        reset_args = False
        if coder.args is None:
            coder.args = SimpleNamespace(debug=False, tui=False)
            reset_args = True
        stream_output = coder.show_send_output_stream(completion)
        try:
            if inspect.isasyncgen(stream_output):
                return [chunk async for chunk in stream_output]
    ...
    @staticmethod
    def _make_model_response(*, finish_reason=None, content=None):
...
This command will print 'Hello, World!' to the console."""
    ...
    async def test_show_send_output_stream_valid_empty_with_finish_reason(self):
        ...
        with GitTemporaryDirectory():
            ...
            mock_response.choices[0].delta.reasoning_content = None
            mock_response.choices[0].delta.reasoning = None
            ...
            await self._drain_show_send_output_stream(coder, mock_response)
    ...
    async def test_show_send_output_response_with_content(self):
        ...
        with GitTemporaryDirectory():
            ...
            # Mock a response with content
            mock_response = self._make_model_response(
                finish_reason="stop", content="Some response content"
            )
