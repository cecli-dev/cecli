...
class Coder:
    ...
    @classmethod
    def _choice_has_finish_reason(cls, choice):
        if not choice:
            return False
        finish_reason = cls._safe_getattr(choice, "finish_reason")
        return bool(finish_reason)

    @classmethod
    def _choice_content_text(cls, choice):
        if not choice:
            return ""

        message = cls._safe_getattr(choice, "message")
        ...
        delta = cls._safe_getattr(choice, "delta")
        content = cls._safe_getattr(delta, "content")
        return content or ""

    @classmethod
    def _choice_has_content(cls, choice):
        if not choice:
            return False
        return bool(cls._choice_content_text(choice))

    @classmethod
    def _choice_has_tool_calls(cls, choice):
        if not choice:
            return False
        message = cls._safe_getattr(choice, "message")
        tool_calls = cls._safe_getattr(message, "tool_calls")
        if tool_calls:
