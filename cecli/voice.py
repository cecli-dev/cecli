...
def _input_device_candidates(sample_rate, device_id):
...
def _sample_rates(sample_rate):
    """Return sample rates to try for a device, preferred first and deduplicated."""
    rates = []
...
def _input_device_error_message(err):
    """Return an actionable message when no microphone input device can be opened."""
    reason = f" ({err})" if err else ""

    return (
        "No microphone input device could be opened"
        + reason
